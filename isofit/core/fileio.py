        ...
        if len(states) == 0:
            ...
            if fm.is_lut_surface:
                if fm.surface.solve_mixed_pixel:
                    full_state_em_idx = np.array(fm.full_idx)[fm.surface.idx_em_rfls]
                    f = fm.surface.softmax(state_est[fm.surface.idx_em_rfls])

                    if "estimated_state_file" in self.output_datasets:
                        to_write["estimated_state_file"][full_state_em_idx] = f

                    if "posterior_uncertainty_file" in self.output_datasets:
                        # Apply delta method transformation
                        J = np.diag(f) - np.outer(f, f)
                        S_frac = J @ S_hat[np.ix_(fm.surface.idx_em_rfls, fm.surface.idx_em_rfls)] @ J.T
                        to_write["posterior_uncertainty_file"][full_state_em_idx] = np.sqrt(np.diag(S_frac))
